...
# save as ${RESULTS_DIR}/<prev>_mean.xyz, then run CPU SA.
#
# If N_WORKERS>1, this script calls `run_parallel.sh` to launch N_WORKERS
# independent CPU processes starting from the same mean geometry.
set -euo pipefail
...
# If set, skip pooling/averaging and use this XYZ as the starting geometry
# (still copied to <prev>_mean.xyz).
...
then run CPU simulated annealing (one run by default; use N_WORKERS>1 to parallelize).
...
Environment (defaults):
  ...
  CONFIG=${CONFIG}
  CPU_CHAINS=${CPU_CHAINS}
  N_WORKERS=${N_WORKERS}  If >1: delegate to run_parallel.sh for concurrency
...
echo "=== CPU run from previous timestep ==="
...
if [[ "$N_WORKERS" -gt 1 ]]; then
    echo "  launching via run_parallel.sh (workers=$N_WORKERS)"
    echo "    STARTING_XYZ=$mean_out"
    echo "    TARGET_FILE=$TARGET_FILE"

    # EXTRA_RUN_PY_ARGS is appended by run_parallel.sh; no quoting to avoid
    # extra quotes becoming part of the argument.
    EXTRA_RUN_PY_ARGS="--config $CONFIG --gpu-backend cpu --gpu-chains $CPU_CHAINS"

    TARGET_FILE="$TARGET_FILE" \
    STARTING_XYZ="$mean_out" \
    N_WORKERS="$N_WORKERS" \
    RESULTS_DIR="$RESULTS_DIR" \
    EXTRA_RUN_PY_ARGS="$EXTRA_RUN_PY_ARGS" \
    "$REPO_ROOT/run_parallel.sh" "$ts_padded" "$excitation_factor" "$tuning_ratio_target"
else
    echo "  launching: $PYTHON run.py --gpu-backend cpu --gpu-chains $CPU_CHAINS ..."
    "$PYTHON" run.py \
        --config "$CONFIG" \
        --run-id "$ts_padded" \
