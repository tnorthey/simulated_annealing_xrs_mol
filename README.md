...
- **Diversity:** multi-chain jobs auto-apply independent Boltzmann starts when no random pool is given (even if `sampling_bool=false`). Across SA restarts and the final GA phase, chains are reseeded from the top `restart_ratio` fraction of previous-phase per-chain bests (ranked by total `f`), tiled onto all chains with carried `f_best` (default `restart_ratio = 1.0` ≡ full previous-best set). Use e.g. `restart_ratio = 0.1` / `--restart-ratio 0.1` for the top 10%. K=1 (global best only) is `restart_ratio` such that `ceil(ratio * gpu_chains) = 1`, e.g. `1/gpu_chains`. For CPU-like ensemble diversity, pass a random XYZ pool via `--gpu-per-chain-random-pool-file` (see `run_cpu_parallel.sh` / `run_gpu_from_previous_timestep.sh`). Final logs include pairwise centroid-aligned RMSD and monitored bond-length spread across chains.
