...
| `scripts/bash/run_gpu_from_previous_timestep.sh` | Start geometry for run *N* from *N−1* (default: Kabsch mean), or `START_FROM_PREVIOUS_RANDOM=1` / `START_FROM_NEXT_RANDOM=1` / `START_FROM_TRADIUS_RANDOM=1` / `START_FROM_TRADIUS_MEAN=1` (random from top `TOP_N`, union pool, or neighbor `{step}_mean.xyz` over *N±1..N±TRADIUS_N*). Multi-chain modes use `--gpu-per-chain-random-pool-file`. See `--help`. |
