...
- If `ab_initio_scattering_file` is set: the file must be two columns `q`, `I` (ab initio scattering at the **reference_xyz** geometry). The code computes IAM for `reference_xyz_file` on **that same q-grid**, writes `results_dir/reference_iam_scattering.dat` for inspection, sets the multiplicative correction to \(I_{\mathrm{ab\ initio}} / I_{\mathrm{IAM}}\), and interpolates it onto the configured `q` grid. In **PCD** mode the factor multiplies **total IAM** before forming PCD; in **non-PCD** mode it multiplies the predicted **total intensity**. Isotropic q only (not with Ewald mode). If unset, the correction factor is unity. The standalone script `calculate_iam.py` supports the same workflow via `--ab-initio-scattering` (requires `--reference`).
