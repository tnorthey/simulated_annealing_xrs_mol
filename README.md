...
| `scripts/bash/run_gpu_from_previous_timestep.sh` | Start geometry for run *N* from timestep *N−1* (default: Kabsch mean of top `TOP_N` → `results/<N−1>_mean.xyz`), or `START_FROM_PREVIOUS_RANDOM=1` / `START_FROM_NEXT_RANDOM=1` for a random pick from top `TOP_N` at *N−1* / *N+1*. One CUDA `run.py` with `GPU_CHAINS` (default 1024). See `--help`. |
