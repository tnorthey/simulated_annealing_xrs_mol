# Simulated Annealing XRS Molecular

Simulated annealing optimization for X-ray scattering molecular structure refinement.

## Installation (via Conda)

```sh
conda create --name sa python=3.10
conda activate sa
conda install -c conda-forge pyscf numba rdkit openbabel openmm openff-toolkit openff-forcefields pytest
```

Optional (CUDA backend):
```sh
conda install -c conda-forge cupy
```

Optional multi-chain CUDA execution (independent SA chains in parallel):
- Set `gpu_backend = "cuda"` and `gpu_chains = <N>` in `input.toml`
- Or use CLI: `python3 run.py --gpu-backend cuda --gpu-chains 8`
- **Diversity:** multi-chain jobs auto-apply independent Boltzmann starts when no random pool is given (even if `sampling_bool=false`). Across SA restarts and the final GA phase, chains are reseeded from the top `restart_ratio` fraction of previous-phase per-chain bests (ranked by total `f`), tiled onto all chains with carried `f_best` (default `restart_ratio = 1.0` ≡ full previous-best set). Use e.g. `restart_ratio = 0.1` / `--restart-ratio 0.1` for the top 10%. Deprecated: `restart_from_global_best_bool = true` (or `--restart-from-global-best`) forces K=1 (global best only). For CPU-like ensemble diversity, pass a random XYZ pool via `--gpu-per-chain-random-pool-file` (see `run_cpu_parallel.sh` / `run_gpu_from_previous_timestep.sh`). Final logs include pairwise centroid-aligned RMSD and monitored bond-length spread across chains.
- Outputs include one `.xyz/.dat` per chain plus (legacy single-chain/CPU) best-structure naming where applicable
- For timestep workflows, `scripts/bash/run_gpu_start.sh` runs an initial step from a chosen XYZ; `scripts/bash/run_gpu_from_previous_timestep.sh` averages the previous step’s best structures into `results/<N−1>_mean.xyz` (when launching run-id `N`), then runs one GPU job (default 1024 chains).
- If `ab_initio_scattering_file` is set: two columns `q`, `I` (ab-initio **total** intensity at **reference_xyz**). The `files.ab_initio_correction_mode` setting selects the denominator: **`elastic`** (default) computes \(c(q)=I_{\mathrm{ab\ initio}}/I_{\mathrm{IAM,\ elastic}}(\mathrm{ref})\), writes `reference_iam_scattering.dat`, and SA uses **\(c\times I_{\mathrm{elastic}}\)**; **`total`** uses \(c(I)=I_{\mathrm{ab\ initio}}/I_{\mathrm{IAM,\ total}}(\mathrm{ref})\) (including Compton when `inelastic` is on), writes `reference_iam_total_scattering.dat`, and SA uses **\(c\times I_{\mathrm{total}}\)** (legacy multiply path). In **PCD** mode, `reference_dat_file` is **required** for the PCD baseline \(I_{\mathrm{ref}}(q)\). Isotropic q only (not with Ewald mode). If unset, \(c\equiv 1\). CLI: `--ab-initio-correction-mode {elastic,total}`. The standalone `scripts/python/calculate_iam.py` has `--ab-initio-correction-mode` (with `--pcd` and `--ab-initio-scattering`, pass `--reference` and `--reference-dat`).

## Quick Start

Run with default configuration:
```bash
python3 run.py
```

Parameters are documented in `input.toml`. Command-line arguments override TOML defaults.

## Signal-only mode (X-ray objective only)

To skip MD/MM priors entirely (no OpenMM system, no SDF creation, no bond/angle/dihedral terms in the SA objective), enable:
- `simulated_annealing_params.signal_only_mode_bool = true`

Alias:
- Setting `simulated_annealing_params.c_tuning_initial = 0` is treated as **equivalent** to `signal_only_mode_bool = true` and prints an informational message.

CLI:
- `python3 run.py --signal-only-mode`

## Scripts

Scripts are organized under `scripts/`:
- `scripts/python/` — Python utility scripts
- `scripts/bash/` — Bash workflow scripts

Each script supports `--help` to display all available options.

| Script | Purpose |
|--------|---------|
| `run.py` | Main simulated annealing optimization |
| `scripts/python/optimal_path.py` | Find smoothest trajectory through candidate structures using DP |
| `scripts/python/compare_random_subsets.py` | Compare multiple random subsets via `optimal_path.py`, analyze geometry, and plot |
| `scripts/python/calculate_iam.py` | Standalone IAM scattering calculation from XYZ files |
| `scripts/python/analyze_geometry.py` | Extract bond lengths, angles, and dihedrals from XYZ files |
| `scripts/python/plot_geometry.py` | Plot geometry CSVs produced by `analyze_geometry.py` |
| `scripts/python/plot_fit_histograms.py` | Histogram of fit values per timestep |
| `scripts/python/average_xyz.py` | Frame-wise averaging of XYZ trajectories |
| `scripts/bash/run_gpu_from_previous_timestep.sh` | Start geometry for run *N* from *N−1* (default: Kabsch mean → `results/<N−1>_mean.xyz`), or random/tradius modes. Missing `{step}_mean.xyz` are auto-built from that step’s top `TOP_N` pool when structures exist. Multi-chain modes use `--gpu-per-chain-random-pool-file`. See `--help`. |
| `scripts/bash/run_cpu_parallel.sh` | Launch `N_WORKERS` parallel `run.py` processes. Three modes: (1) `STARTING_XYZ` set — fixed file for every worker; (2) `START_FROM_PREV_MEAN=1` — Kabsch mean of `TOP_N` best from previous step; (3) default — random pick from `XYZ_SOURCE_STEP` pool. See `--help`. |
| `scripts/bash/run_gpu_start.sh` | First (or any) timestep from a **fixed** starting XYZ: copies it to `results/NN_mean.xyz`, then same CUDA `run.py` as above. Pass `starting_xyz` as the second argument, or set `STARTING_XYZ`. See `--help`. |
| `scripts/bash/run_chd_ewald_test.sh` | CHD test+Ewald run: generates target I(q,θ,φ) from `xyz/target_20.xyz` via IAM on a 3D Ewald grid, SA starts from `xyz/start.xyz`. Config: `input_chd_ewald.toml`. Outputs `target_function.npy` (3D) and per-worker `.xyz` results. Env: `N_WORKERS`, `RESULTS_DIR`. |

## Testing

```bash
pytest tests/
```
