...
- **Diversity:** multi-chain jobs auto-apply independent Boltzmann starts when no random pool is given (even if `sampling_bool=false`). Across SA restarts and the final GA phase, each chain continues from **its own** previous best coordinates by default (not a single global best cloned to all chains). Set `restart_from_global_best_bool = true` (or `--restart-from-global-best`) to restore the older policy of reseeding every chain from the single global best after each phase. For CPU-like ensemble diversity, pass a random XYZ pool via `--gpu-per-chain-random-pool-file` (see `run_cpu_parallel.sh` / `run_gpu_from_previous_timestep.sh`). Final logs include pairwise centroid-aligned RMSD and monitored bond-length spread across chains.
