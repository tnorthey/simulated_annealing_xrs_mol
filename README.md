...
- If `ab_initio_scattering_file` is set: two columns `q`, `I` (ab-initio **total** intensity at **reference_xyz**). The code computes **elastic** IAM for `reference_xyz_file` on that q-grid, writes `results_dir/reference_iam_scattering.dat`, sets \(c(q)=I_{\mathrm{ab\ initio}}/I_{\mathrm{IAM,\ elastic}}(\mathrm{ref})\), and interpolates \(c(q)\) onto the configured `q` grid. SA uses **\(c(q)\times I_{\mathrm{elastic}}\)** (atomic + molecular) for the predicted curve. In **PCD** mode, `reference_dat_file` is **required**: the PCD denominator \(I_{\mathrm{ref}}(q)\) is read from that DAT file (not recomputed IAM). Isotropic q only (not with Ewald mode). If unset, \(c(q)\equiv 1\). The standalone script `calculate_iam.py` mirrors this (with `--pcd` and `--ab-initio-scattering`, pass `--reference` and `--reference-dat`).
