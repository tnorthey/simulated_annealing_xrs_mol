...
class TestInputToParams:
    ...
    def test_read_valid_toml(self, sample_toml_file):
        ...
        assert p.reference_dat_file is None
        assert p.ab_initio_correction_mode == "elastic"

    def test_empty_optional_file_overrides_are_none(self, sample_toml_file):
        """CLI '' must clear optional DAT paths, not stringify to 'None'."""
        p = Input_to_params(
            sample_toml_file,
            overrides={
        ...
        assert p.reference_dat_file is None
        assert p.ab_initio_scattering_file is None
        p_none_str = Input_to_params(
            sample_toml_file,
            overrides={"files.reference_dat_file": "None"},
        )
        assert p_none_str.reference_dat_file is None
